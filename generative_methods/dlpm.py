...
class DLPM:
    def __init__(
        ...
        self.gen_a = CustomDistributionDataset('skewed_levy', 
                                    alpha = self.alpha, 
                                    device=self.device,
                                    isotropic=isotropic,
                                    clamp_a = clamp_a)
        
        self.gen_eps = CustomDistributionDataset('sas',
                                      alpha = self.alpha, 
                                      device=self.device,
                                      isotropic=isotropic,
                                      clamp_eps = clamp_eps)
...
#######################################################################################
    ...
    def predict_score_from_eps(self, x_t, t, eps):
        g, bg, s, bs = self.update_constants(x_t.shape)
        t = self.get_t_to_batch_size(x_t, t)
        score = - eps / bs[t]
        return score
    
    def predict_ode_drift_from_eps(self, x_t, t, eps):
        g, bg, s, bs = self.update_constants(x_t.shape)
        t = self.get_t_to_batch_size(x_t, t)
        score = - eps / bs[t]
        
        ode_drift = bg[t] - bs[t]**2 * score / 2
